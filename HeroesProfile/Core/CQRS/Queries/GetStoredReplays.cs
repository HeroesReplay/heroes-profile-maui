...
public static class GetReplays
{
    ...
    public class Handler(ReplaysRepository repository) : IRequestHandler<Query, Response>
    {
        public async Task<Response> Handle(Query request, CancellationToken cancellationToken)
        {
            ...
            return new Response(replays.Where(replay => request.Filters.Any(filter => replay.ProcessStatus == filter.ProcessStatus && replay.ParseStatus == filter.ParseStatus)).ToList());
        }
    }
}