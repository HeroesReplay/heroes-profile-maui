...
namespace HeroesProfile.UI.Core;
...
public static class PollyPolicies
{
    ...
    private static void OnRetry<T>(DelegateResult<HttpResponseMessage> dr, TimeSpan timeSpan, int retryAttempt, Context context, ILogger<T> logger)
    {
        ...
        {
            logger.LogInformation("Setting RetryAfter: {RetryAfterDelta}", dr.Result.Headers.RetryAfter.Delta.Value);
    ...
    {
        if (context.TryGetValue(RetryAfterKey, out var value))
        {
